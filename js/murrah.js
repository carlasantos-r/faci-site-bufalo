...
document.getElementById("murrah3d").appendChild(renderer.domElement);
...
loader.load(
  ...
  function (error) {
    //Se houver algum erro, registre-o
    console.error(error);
  }
);
...
const leftLight = new THREE.DirectionalLight(0xffffff, 1); // (cor, intensidade)
leftLight.position.set(-500, 500, 500) 
leftLight.castShadow = true;
scene.add(leftLight);

const rightLight = new THREE.DirectionalLight(0xffffff, 1); // (cor, intensidade)
rightLight.position.set(500, 500, 500) 
rightLight.castShadow = true;
scene.add(rightLight);

const underLight = new THREE.DirectionalLight(0xffffff, 2); // (cor, intensidade)
